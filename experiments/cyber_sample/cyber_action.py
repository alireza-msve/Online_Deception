...
class CyberAction(DiscreteAction):
    """
    -The cyber sample problem Action class
    -Wrapper for storing the bin number. Also stores the cyber number for checking actions
    -Handles pretty printing
    """

    def __init__(self, bin_number):
        super(CyberAction, self).__init__(bin_number)
        if self.bin_number >= ActionType.CHECK:
            self.cyber_no = self.bin_number - ActionType.CHECK
        else:
            self.cyber_no = 0

    def copy(self):
        return CyberAction(self.bin_number)
